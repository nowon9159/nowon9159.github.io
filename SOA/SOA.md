...
S3 데이터 전송 요금의 경우
- S3 버킷으로 데이터를 전송한느 것은 수신 트래픽이므로 무료이다.
...
하나의 거대한 모놀리식 애플리케이션이라면 디버깅이 비교적 쉽지만, 분산 서비스들이 SQS 큐 또는 SNS Topic을 통해 연결되어 있고 분리되어 있다면 시스템 내부에서 발생한느 일을 추적하고 파악하기가 매우 어려워진다.
