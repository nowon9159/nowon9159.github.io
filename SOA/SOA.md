...
- SQL 문에 의해 필터링할 수도 있다.
  - 기본적으로 SQL 문이 데이터베이스를 차단하거나 느리게 만들고 있는 경우 해당 SQL 문을 식별할 수 있으며 그 원인을 파악하기 위해 그 SQL문을 실행한느 팀 또는 애플리케이션에 연락해 최적화할 수 있다.
- 호스트 별로 필터링할 수 있다.
...
Aurora는 백트랙이라는 기능을 제공하는데, 언제든지 데이터를 복원할 수 있는 능력이다.
