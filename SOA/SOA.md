...
S3 이벤트 알림의 경우 S3 버킷에 파일을 업로드하면 새 파일 이벤트가 생성되어 Lambda 함수를 호출하고 동시에 많은 파일을 업로드한다고 가정해보자.

수 많은 다양한 Lambda 동시 실행이 발생할 것이고, 함수에 사용 가능한 동시성이 충분하지 않은 경우 추가 요청이 스로틀링될 것이다.
...
콜드 스타트는 새로운 람다 함수 인스턴스를 생성할 때 코드를 로드하고 핸들러 외부의 코드를 실행해야 한다는 뜻이다. 이는 모든 init에 해당한다.
