...
- Multi AZ
  ...
  - 마스터에 대한 모든 변경 사항이 동기식으로 대기 인스턴스에 복제된다.
  - 애플리케이션은 하나의 DNS 이름으로 접속하여 마스터에 문제가 생기면 자동으로 Standby 데이터베이스로 fail over 된다. 이렇게 가용성이 향상되어 Multi AZ라고 부른다.
  - 애플리케이션 관점에서는 수동 개입 없이 자동으로 데이터베이스 연결을 시도하면 새 마스터로 페일오버 된다.
  - Multi AZ는 확장을 위한것이 아니며, 페일오버할 목적으로만 존재한다.
